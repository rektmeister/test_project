  ...
  <div class="layout-page" v-css="{ 'background-image': 'url(' + bgImgUrl + ')' }">
    ...
    <transition name="fade">
      <main v-show="showMain" class="co-messager-layout">
        ...
        <el-main :class="device === 'Mobile' ? 'co-messager-main mobile' : 'co-messager-main'"
...
export default {
  ...
  mounted() {
    ...
    console.log(
      `%c Version: ${APP_VERSION} %c Contact: 3043777100@qq.com %c`,
  ...
  created() {
    ...
    this.$socket.on("receiveMessage", (news) => {
      if (news.senderId !== this.userInfo._id) {
        var text = this.$rsa_decrypt(news["message"])
        news.message = text
      ...
      this.$socket.on("receiveValidateMessage", (news) => {
      ...
      this.$socket.on("receiveAgreeFriendValidate", (data) => {
