  ...
  <div class="chat-area__com">
    ...
    <div class="message-edit-container">
      ...
      <div class="operation">
        ...
        <el-button @click="messageText = ''" type="danger" size="small" round>清空</el-button>
      </div>
  ...
  methods: {
    ...
    fileInpChange(e) {
      ...
      this.$http.uploadFile(formdata).then(res => {
        ...
        var obj = JSON.parse(JSON.stringify(newMessage))
        let text = this.$rsa_encrypt(obj.message)
        obj.message = text
        this.$socket.emit("sendNewMessage", obj)
      })
    },
    addEmoji(emoji = '') {
      ...
      if (conversationType === conversationTypes.friend) {
        ...
        if (data.status === 2000 && status === 200) {
          data.data.forEach(element => {
            element.message = this.$rsa_decrypt(element.message)
          });
